...
# Shrink speed scaling
scoreboard players operation $temp_2 explosivebarrels = $temp_0 explosivebarrels
scoreboard players set $temp_3 explosivebarrels 5
scoreboard players operation $temp_2 explosivebarrels *= $temp_3 explosivebarrels
scoreboard players set $temp_3 explosivebarrels 3
scoreboard players operation $temp_2 explosivebarrels /= $temp_3 explosivebarrels

# Primary cloud setup
execute if score $temp_1 explosivebarrels matches 1..2 run summon area_effect_cloud ~ ~ ~ {Tags:[explosivebarrel_0],Duration:600}
...
execute if score $temp_1 explosivebarrels matches 1..2 store result entity @e[type=area_effect_cloud,tag=explosivebarrel_0,limit=1,sort=nearest] RadiusPerTick float -0.001 run scoreboard players get $temp_2 explosivebarrels
...
execute if score $temp_1 explosivebarrels matches 2..3 run summon area_effect_cloud ~ ~ ~ {Tags:[explosivebarrel_1],Duration:600}
...
execute if score $temp_1 explosivebarrels matches 2..3 store result entity @e[type=area_effect_cloud,tag=explosivebarrel_1,limit=1,sort=nearest] RadiusPerTick float -0.001 run scoreboard players get $temp_2 explosivebarrels
